...
namespace ElementsLib.Elements.Config.Articles
{
    ...
    using SourceItem = Sources.TaxBaseAdvanceHealthSource;
    ...
    public class TaxBaseAdvanceHealthArticle : GeneralArticle, ICloneable
    {
        ...
        public static string ARTICLE_DESCRIPTION_ERROR_FORMAT = "TaxBaseAdvanceHealthArticle(ARTICLE_TAX_BASE_ADVANCE_HEALTH, 1015): {0}";

        public TaxBaseAdvanceHealthArticle() : base((ConfigRole)ConfigRoleEnum.ARTICLE_TAX_BASE_ADVANCE_HEALTH)
        {
            SourceValues = new TaxBaseAdvanceHealthSource();

            InternalEvaluate = TaxBaseAdvanceHealthConcept.EvaluateConcept;
        }

        public TaxBaseAdvanceHealthArticle(ISourceValues values) : this()
        {
            TaxBaseAdvanceHealthSource sourceValues = values as TaxBaseAdvanceHealthSource;

            SourceValues = CloneUtils<TaxBaseAdvanceHealthSource>.CloneOrNull(sourceValues);
        }
        ...
        public TaxBaseAdvanceHealthSource SourceValues { get; set; }

        public override void ImportSourceValues(ISourceValues values)
        {
            SourceValues = SetSourceValues<TaxBaseAdvanceHealthSource>(values);
        }
        ...
        {
            TaxBaseAdvanceHealthArticle cloneArticle = (TaxBaseAdvanceHealthArticle)this.MemberwiseClone();
